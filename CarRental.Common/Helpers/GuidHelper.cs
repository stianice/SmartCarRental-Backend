...
namespace CarRental.Common.Helpers;
    ...
    public static Guid Next(SequentialGuidType guidType)
    {
        ...
        switch (guidType)
        {
            ...
            case SequentialGuidType.AsBinary:
                ...
                guidBytes[6] = (byte)(
                    (Rfc4122Version4 << 4) | ((timestampBytes[6] & FilterLowBit) >> 4)
                );
                // dd : 高4位取：[6]低4位 | 低4位取：[7]高4位
                guidBytes[7] = (byte)(
                    ((timestampBytes[6] & FilterHighBit) << 4)
                    | ((timestampBytes[7] & FilterLowBit) >> 4)
                );
            ...
            case SequentialGuidType.AtEnd:
                ...
                guidBytes[7] = (byte)(
                    ((timestampBytes[7] & FilterHighBit) << 4) | (randomBytes[7] & FilterHighBit)
                );
                // Nd : 高4位为：变体 | 低4位取：时间戳[6]高4位
                guidBytes[8] = (byte)(
                    (Rfc4122Variant << 4) | ((timestampBytes[6] & FilterLowBit) >> 4)
                );
